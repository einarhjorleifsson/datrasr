...
#' @export
 ...
 #what.species = 'Solea solea'
  ...
  shootlong=lfdat$shootlong,shootlat=lfdat$shootlat),sum,na.rm=T)    #sum nos and weights over length, year, qtr, and statrectangle
  ...
  list(year=slfs$year,quarter=slfs$quarter,age=slfs$age,shootlong=slfs$shootlong,shootlat=slfs$shootlat),sum,na.rm=T)
  ...
  haulduration <- aggregate(list(haulduration=chrons$hauldur), list(year=chrons$year,shootlong=chrons$shootlong,shootlat=chrons$shootlat,quarter=chrons$quarter),sum,na.rm=T)
